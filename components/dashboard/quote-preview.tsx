...
function parse(quote: Quote | null): ParsedScope {
  if (!quote?.scopeOfWork) return {};
  try {
    return JSON.parse(quote.scopeOfWork) as ParsedScope;
  } catch {
    return { plainTextScope: quote.scopeOfWork };
  }
}
      ...
      <article className="mt-5 rounded-2xl bg-paper-document p-8 text-paper-ink-strong shadow-xl">
        <header className="flex items-start justify-between border-b border-paper-rule pb-4">
          <div>
            ...
            <p className="text-sm text-paper-ink-faint">Roofing quote</p>
          </div>
        ...
        <div className="mt-5 flex items-baseline justify-between">
          <h4 className="text-xl font-semibold text-paper-ink">{quote.title}</h4>
          {typeof quote.totalAmountCents === "number" && (

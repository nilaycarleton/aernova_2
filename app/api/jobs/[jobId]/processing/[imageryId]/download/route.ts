...
function cacheKey(jobId: string, imageryId: string, fileName: string) {
  return `processing/${jobId}/${imageryId}/${fileName}`;
}
...
async function cachedAssetBytes(jobId: string, imageryId: string, fileName: string) {
  return storage.getBytes(cacheKey(jobId, imageryId, fileName));
}
...
async function getAllZipBytes(jobId: string, imageryId: string, taskUuid: string) {
  const cached = await storage.getBytes(cacheKey(jobId, imageryId, "all.zip"));
  if (cached) return cached;
  const bytes = await downloadNodeOdmAllZip(taskUuid);
  await storage.put(cacheKey(jobId, imageryId, "all.zip"), bytes, "application/zip");
  return bytes;
}
...
export async function GET(
  ...
  try {
    ...
    if (requestedAsset === "viewerGlb") {
      ...
      const cachedViewer = await cachedAssetBytes(jobId, imageryId, viewerFileName);
      if (cachedViewer) {
      ...
      const fullBytes =
        localFullModel ??
        extractZipEntry(await getAllZipBytes(jobId, imageryId, taskUuid), nodeOdmOutputAssets.texturedGlb);
      if (!fullBytes) {
        return NextResponse.json({ error: "Textured GLB not found in worker output" }, { status: 404 });
      }
      await storage.put(cacheKey(jobId, imageryId, viewerFileName), fullBytes, "model/gltf-binary");
    ...
    if (requestedAsset === "all") {
      const zip = await getAllZipBytes(jobId, imageryId, taskUuid);
      return new Response(new Uint8Array(zip), { status: 200, headers });
    }

    const cached = await cachedAssetBytes(jobId, imageryId, fileName);
    if (cached) {
    ...
    const bytes =
      ...
      extractZipEntry(await getAllZipBytes(jobId, imageryId, taskUuid), assetPath);
    if (!bytes) {
      return NextResponse.json({ error: `Asset ${requestedAsset} not found in worker output` }, { status: 404 });
    }
    await storage.put(cacheKey(jobId, imageryId, fileName), bytes, contentTypeFor(fileName));

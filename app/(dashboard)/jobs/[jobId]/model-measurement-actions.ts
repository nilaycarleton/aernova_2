...
export async function saveModelMeasurementAction(input: ModelMeasurementInput) {
  await requireJobAccess(input.jobId, "editJob");
...
export async function updateModelMeasurementCategoryAction(input: {
  ...
  await prisma.modelMeasurement.updateMany({
    where: { id: input.id, jobId: input.jobId },
    data: { category },
  });
  revalidatePath(`/jobs/${input.jobId}`);
}
...
/**
 * Replace the job's entire model-measurement set with the given list, in one
...
export async function replaceModelMeasurementsAction(input: {
  ...
  await prisma.$transaction([
    prisma.modelMeasurement.deleteMany({ where: { jobId: input.jobId } }),
    ...(rows.length ? [prisma.modelMeasurement.createMany({ data: rows })] : []),
  ]);
  revalidatePath(`/jobs/${input.jobId}`);
}

export async function deleteModelMeasurementAction(input: { jobId: string; id: string }) {
  await requireJobAccess(input.jobId, "editJob");
  await prisma.modelMeasurement.deleteMany({ where: { id: input.id, jobId: input.jobId } });
  revalidatePath(`/jobs/${input.jobId}`);
}

export async function clearModelMeasurementsAction(input: { jobId: string }) {
  await requireJobAccess(input.jobId, "editJob");
  await prisma.modelMeasurement.deleteMany({ where: { jobId: input.jobId } });
  revalidatePath(`/jobs/${input.jobId}`);
}
...
export async function autoDetectRoofFacetsAction(input: {
  ...
  const mesh = await resolveTexturedModelMesh(input.jobId, input.imageryId);
 ...
 */
export async function classifyRoofEdgesAction(input: { jobId: string }): Promise<ClassifiedLine[]> {
  await requireJobAccess(input.jobId, "editJob");

  const rows = await prisma.modelMeasurement.findMany({ where: { jobId: input.jobId, kind: "area" } });
  ...
  await prisma.modelMeasurement.deleteMany({
    where: { jobId: input.jobId, label: { startsWith: EDGE_LABEL_PREFIX } },
  });
  if (edges.length === 0) {
    revalidatePath(`/jobs/${input.jobId}`);
    return [];
...
/**
 * Turn the measurements drawn on the model into a roofing quote: area polygons
 ...
 * then a DRAFT Quote is created and opened.
 */
export async function generateEstimateFromMeasurementsAction(formData: FormData) {
  ...
  const rows = await prisma.modelMeasurement.findMany({ where: { jobId } });
  ...
  const report = generateRoofingReport(job, synthetic, []);
  const quote = await withQuoteNumber(companyId, (quoteNumber) =>
    ...
    data: {
      ...
      title: report.title,
      status: "DRAFT",
      // Dual-written while the legacy float column is retired — see lib/money.ts.
      totalAmountCents: toCents(report.totalAmount),
  ...
  redirect(`/jobs/${jobId}/quotes/${quote.id}`);
}
...
/**
 * Materialise the drawn measurements into the job's standard Measurement
 * rows so they appear in the measurements panel (and the classic quote
 * generator), not just the 3D viewer. Idempotent — replaces the previous
 * "Model roof:" rows each time.
 */
export async function saveModelMeasurementsToJobAction(formData: FormData) {
  const jobId = String(formData.get("jobId") ?? "").trim();
  ...
  const base = { jobId, source: CaptureSource.DRONE, confidence: null };
  ...
  await prisma.$transaction([
    prisma.measurement.deleteMany({ where: { jobId, label: { startsWith: MODEL_MEASUREMENT_LABEL_PREFIX } } }),

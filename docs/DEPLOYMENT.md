...
| `GEMINI_API_KEY` | Can likely stay the same key (Google AI Studio doesn't separate test/live for the Gemini API), but worth confirming it has production-appropriate rate limits/spend controls set in the Google AI Studio / Google Cloud console. (Migrated from Anthropic — see `docs/PLAN-CRM.md`'s AI section for the migration date.) |
